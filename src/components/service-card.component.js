export class ServiceCard extends HTMLElement {
    template() {
        return `
            <article 
                id='serviceCard' 
                class="p-4 shadow-sm h-100 d-flex align-items-start flex-column"
                data-mdb-animation-init
                data-mdb-animation="fade-in-down"
                data-mdb-animation-start="onScroll"
            >
                <div class="border p-2 bg-primary bg-opacity-75 d-inline-block rounded-3">
                    <img width="50px" src="${this.attributes.img.value}" alt="">
                </div>
                <h3 class="mt-4 mb-2 h4 fw-semibold">${this.attributes.title.value}</h3>
